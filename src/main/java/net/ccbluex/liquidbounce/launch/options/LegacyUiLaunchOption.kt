    ...
    override fun start() {
        ...
        Thread {
            ...
            if (LiquidBounce.CLIENT_VERSION == LiquidBounce.CLIENT_REAL_VERSION && !latestVersion.equals(LiquidBounce.CLIENT_VERSION)) {
                val buttons =
                    arrayOf(LanguageManager.get("ui.update.download"), LanguageManager.get("ui.update.dismiss"))
                val selection = JOptionPane.showOptionDialog(
                    null, LanguageManager.getAndFormat("ui.update.released", latestVersion), "Alert",
