...
class Targets : Element(-46.0, -40.0, 1F, Side(Side.Horizontal.MIDDLE, Side.Vertical.MIDDLE)) {
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        when (switchModeValue.get().lowercase()) {
            "zoom" -> {
                ...
                GL11.glTranslatef(
                    ((border.x2 * 0.5f * (1 - displayPercent)) / displayPercent).toFloat(),
                    ((border.y2 * 0.5f * (1 - displayPercent)) / displayPercent).toFloat(),
                    0f
                )
            }
            "slide" -> {
    ...
    private fun drawNovo(target: EntityLivingBase, nowAnimHP: Float) {
        ...
        RenderUtils.drawRect(
            hpPos,
            18F,
            33F + ((nowAnimHP / target.maxHealth * 10000).roundToInt() / 100),
            25F,
            darkColor
        )
    ...
    private fun drawRise(target: EntityLivingBase, easingHealth: Float) {
        ...
        val skin =
            mc.netHandler.getPlayerInfo(target.uniqueID)?.locationSkin ?: DefaultPlayerSkin.getDefaultSkinLegacy()
        ...
        fun renderSideway(x: Int, x1: Int) {
            RenderUtils.quickDrawGradientSideways(
                ...
                x1.toDouble(),
                45.0,
                ColorUtils.hslRainbow(x, indexOffset = 10).rgb,
                ColorUtils.hslRainbow(x1, indexOffset = 10).rgb
            )
        }
    ...
    private fun getTBorder(): Border? {
        return when (modeValue.get().lowercase()) {
            ...
            "liquid" -> Border(
                0F, 0F, (38 + mc.thePlayer.name.let(Fonts.font20::getStringWidth)).coerceAtLeast(118).toFloat(), 36F
            )
            "flux" -> Border(
                0F, 0F, (38 + mc.thePlayer.name.let(Fonts.font20::getStringWidth))
                    .coerceAtLeast(70)
                    .toFloat(), 34F
            )
