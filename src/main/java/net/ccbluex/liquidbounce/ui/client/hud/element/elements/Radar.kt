...
class Radar(x: Double = 5.0, y: Double = 130.0) : Element(x, y) {
    ...
    override fun drawElement(): Border? {
        ...
        if (!minimapValue.get()) {
            if (skeetRectValue.get()) drawExhiRect(0F, 0F, size, size)
            else RenderUtils.drawRect(0F, 0F, size, size, Color(backgroundRedValue.get(), backgroundGreenValue.get(),
        ...
        if (borderValue.get()) RainbowShader.begin(borderRainbowValue.get(), if (rainbowXValue.get() == 0.0F) 0.0F else 1.0F / rainbowXValue.get(),
