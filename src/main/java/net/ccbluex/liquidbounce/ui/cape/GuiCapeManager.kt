...
object GuiCapeManager : GuiScreen() {
        ...
        for (file in LiquidBounce.fileManager.capesDir.listFiles()) {
            if (file.isFile && !file.name.equals(jsonFile.name)) {
                try {
                    ...
                    capeList.add(
                        loadCapeFromFile(
                            java.lang.String.join(".", *args.copyOfRange(0, args.size - 1)),
                            file
                        )
                    )
                } catch (e: Exception) {
    ...
    private fun loadCapeFromResource(name: String, loc: String) =
        Cape(name, ImageIO.read(GuiCapeManager::class.java.classLoader.getResourceAsStream(loc)))
    ...
    override fun initGui() {
        ...
        this.buttonList.add(
            GuiButton(
                ...
                (width * 0.3).toInt(),
                (height * 0.5).toInt(),
                Fonts.font20.getStringWidth("<-") + 10,
                20,
                "<-"
            )
        )
