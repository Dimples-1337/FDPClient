...
class ConfigManager {
    ...
    fun loadConfigSet() {
        val configSet = if (configSetFile.exists()) {
            JsonParser().parse(IOUtils.toString(FileInputStream(configSetFile), "utf-8")).asJsonObject
        } else {
            JsonObject()
        }
    ...
    private fun registerSection(sectionClass: Class<out ConfigSection>) {
        try {
            ...
            ClientUtils.getLogger()
                .error("Failed to load config section: ${sectionClass.name} (${e.javaClass.name}: ${e.message})")
        }
    }
}