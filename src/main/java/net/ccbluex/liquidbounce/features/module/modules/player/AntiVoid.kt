...
class AntiVoid : Module() {
    private val modeValue =
        ListValue("Mode", arrayOf("Blink", "TPBack", "FlyFlag", "GroundSpoof", "TestHypixel"), "Blink")
        ...
        when (modeValue.get().lowercase()) {
            ...
            "testhypixel" -> {
                if (packet is C03PacketPlayer) {
                    if (voidOnly.get() && mc.thePlayer.fallDistance >= maxFallDistValue.get() && mc.thePlayer.motionY <= 0 && mc.theWorld.getCollidingBoundingBoxes(
